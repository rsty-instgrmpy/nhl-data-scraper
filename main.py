...
if __name__ == "__main__":
    ...
    for row in json['teams']:
        ...
        for year in [2014,2015,2016,2017]:
            playerCounter = 0
            try:         
                roster = scraper.getPlayersFromTeam(row['id'], getSeason(year)).json()['roster']
                # while playerCounter < 4:
                for player in roster:
                    if playerCounter < 40:
                        print(player)
                        playerStats = scraper.getPlayerStats(player['person']['id'], getSeason(year)).json()['people'][0]
                        ...
                        if playerStats['primaryPosition']['code'] == 'G':
                            p = scraper.playerSeasonRecord(playerID = playerStats['id'], firstName = playerStats['firstName'], lastName = playerStats['lastName'] ,\
                                position = playerStats['primaryPosition']['abbreviation'], birthDate = playerStats['birthDate'], seasonTeam = teamName, \
                                #### stats ####
                            ...
                            try:
                                p = p._replace(shortHandedSavePercentage = playerStats['stats'][0]['splits'][0]['stat']['shortHandedSavePercentage'])
                            except KeyError:
                                pass

                            try:
                                p = p._replace(powerPlaySavePercentage = playerStats['stats'][0]['splits'][0]['stat']['powerPlaySavePercentage'])
                            except KeyError:
                                pass
                            ...
                            p = scraper.playerSeasonRecord(playerID = playerStats['id'],\
                                ...
                                birthDate = playerStats['birthDate'] , seasonTeam = teamName,\
